"""
Generate NEGATIVE exemplar annotations using OWL-v2 (Google).
Mirrors owlv2_pos.py but queries with negative class descriptions and
inverts the binary-classifier filter so only non-target patches are kept.
...
# ---------------------------------------------------------------------------
# Binary classifier (identical to owlv2_pos.py)
# ---------------------------------------------------------------------------
...
def is_negative_patch(patch, classifier, preprocess, threshold=BINARY_THRESHOLD):
    ...
    Inverted from is_valid_patch in owlv2_pos.py:
...
# ---------------------------------------------------------------------------
# OWL-v2 detection (identical to owlv2_pos.py)
# ---------------------------------------------------------------------------
def detect_owlv2(image_pil, text_queries, processor, model, threshold=SCORE_THRESHOLD):
    """
    Args:
        image_pil: PIL Image
        text_queries: list of strings, e.g. ["not apple", "background near apple"]
    Returns:
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate negative exemplar annotations with OWL-v2"
    )
    ...
    parser.add_argument("--output_file", required=True,
                        help="Output JSON annotation file "
                             "(e.g. annotation_FSC147_neg_owlv2.json)")
    ...
    for i, img_name in enumerate(image_files, 1):
        ...
        # Build negative text queries
        if args.prompt and img_name in prompt_map:
            queries = prompt_map[img_name].get("negative_descriptions", [])
            if not queries:
                queries = [f"not {singular}", f"background near {singular}"]
        else:
            queries = [
                f"not {singular}",
                f"background near {singular}",
                f"similar to {singular}",
            ]
