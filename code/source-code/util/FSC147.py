...
class ResizeTrainImage(ResizeSomeImage):
    ...
    def __call__(self, sample):
        ...
        # if len(boxes) > 0:
        #     boxes = torch.stack(boxes)  # If boxes is non-empty, execute torch.stack normally
        #     boxes1 = boxes
        # else:
        #     boxes = boxes1
        #     pass
        # # If boxes is empty, you can skip this sample or provide a default bounding box
        # # For example, use a default box representing the full image area
        #     default_box = torch.tensor([[0, 0],[0, 0],0, 0])  # Example default bbox; exact values depend on your use case
        #     boxes = default_box.unsqueeze(0)  # Add a dimension to match torch.stack requirements
        #     # pass
        if aug_flag:
            pos = torch.tensor([])
        else:
