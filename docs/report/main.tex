...
\sujet{CÁC KỸ THUẬT HỌC SÂU VÀ ỨNG DỤNG -- CS431.Q22 \\ BÁO CÁO ĐỒ ÁN}
\UE{ }
\enseignant{TS. Nguyễn Vinh Tiệp}
...
Báo cáo này trình bày kết quả nghiên cứu và hiện thực hệ thống \textbf{VA-Count}~\cite{vacount} -- một phương pháp \textit{zero-shot object counting} sử dụng exemplar tốt để đếm đối tượng trong ảnh mà không cần cung cấp mẫu thủ công. Hệ thống nhận đầu vào là một ảnh RGB và một văn bản mô tả lớp đối tượng cần đếm, sau đó tự động sinh mẫu qua module \textit{Exemplar Enhancement Module} (EEM) và dự đoán số lượng qua module \textit{Noise Suppression Module} (NSM) với cơ chế học tương phản.

Nhóm mở rộng VA-Count theo hai hướng độc lập: (i)~tích hợp \textit{Rich Prompts} -- dùng mô hình ngôn ngữ đa phương thức (Gemini) để sinh mô tả phong phú kết hợp CLIP re-ranking nhằm cải thiện chất lượng exemplar; và (ii)~thay thế GroundingDINO bằng \textit{YOLO-World} để giải quyết nút thắt cổ chai về tốc độ (chiếm $\sim$80\% thời gian pipeline). Kết quả đánh giá trên bộ dữ liệu FSC-147 với bốn cấu hình cho thấy MAE dao động từ $17{,}80$ đến $19{,}03$, trong đó cấu hình YOLO-World + Rich Prompt ($17{,}91$) gần như bắt kịp baseline ($17{,}99$) với tốc độ suy luận nhanh hơn $\approx$2,5$\times$.

Một phát hiện quan trọng của đồ án là \textbf{giả thuyết bão hòa ngữ nghĩa}: bộ phát hiện mạnh về ngữ nghĩa (GroundingDINO) hưởng lợi rất ít từ Rich Prompt ($\Delta\mathrm{MAE} = 0{,}19$), trong khi bộ phát hiện nhẹ hơn (YOLO-World) hưởng lợi đáng kể ($\Delta\mathrm{MAE} = 1{,}12$). Ngoài ra, nhóm chỉ ra tính chất \textit{prompt faithfulness} của YOLO-World: mô hình chỉ phát hiện đối tượng khi prompt thực sự khớp với nội dung ảnh, trái với GroundingDINO vốn có thể trả về kết quả không trung thực với prompt đầu vào.

%---- LỜI NÓI ĐẦU ------
\chapter*{LỜI NÓI ĐẦU}
\addcontentsline{toc}{chapter}{LỜI NÓI ĐẦU}

Lời đầu tiên, nhóm chúng em xin bày tỏ lòng biết ơn sâu sắc đến cô \textbf{TS. Nguyễn Vinh Tiệp}, người đã tận tình hướng dẫn, định hướng và truyền đạt những kiến thức quý báu trong môn học \textit{Kỹ thuật học sâu và ứng dụng (CS431.Q22)}. Sự góp ý chuyên môn và những buổi trao đổi của cô đã giúp nhóm hiểu rõ hơn về bài toán đếm đối tượng zero-shot, hệ thống VA-Count cũng như cách đánh giá khoa học một mô hình thị giác máy tính trên bộ dữ liệu FSC147.\\[1em]
...
\hline
Nguyễn Lê Phong & 23521168 & 33\% & Thành viên &
...
\begin{itemize}
    \item \textbf{Chương~1 -- Tổng quan đề tài} (chương này): mô tả bài toán zero-shot object counting, lý do chọn VA-Count làm baseline, phát biểu chính thức bài toán, các đóng góp của đồ án và bố cục tài liệu.
    \item \textbf{Chương~2 -- Phương pháp thực hiện}: trình bày kiến trúc hệ thống VA-Count gốc (EEM + NSM), hai hướng mở rộng (Rich Prompts và YOLO-World) cùng chi tiết cài đặt và bảng tổng hợp bốn cấu hình thực nghiệm.
    \item \textbf{Chương~3 -- Thực nghiệm và kết quả}: mô tả bộ dữ liệu FSC-147, độ đo đánh giá (MAE, RMSE), thiết lập thực nghiệm, so sánh định lượng bốn cấu hình và phân tích lỗi sai.
    \item \textbf{Chương~4 -- Demo và đánh giá hệ thống}: hướng dẫn cài đặt và chạy ứng dụng Streamlit demo, mô tả giao diện người dùng và đánh giá định tính trên các ảnh thực tế.
    \item \textbf{Chương~5 -- Kết luận và hướng phát triển}: tổng kết kết quả, phân tích giả thuyết bão hòa ngữ nghĩa và đề xuất các hướng cải tiến trong tương lai.
\end{itemize}

\bigskip
\noindent\textit{\textbf{Kết chương~1.} Chương này đã thiết lập nền tảng lý luận cho toàn bộ đồ án: bài toán đếm zero-shot, lý do chọn VA-Count, các đóng góp và bố cục tài liệu. Chương~2 tiếp theo sẽ trình bày chi tiết kiến trúc hệ thống và hai hướng mở rộng được đề xuất.}
...
2. Format: Start with 'single {class_name}'.
...
\end{itemize}
Ngoài vấn đề tốc độ, nhóm nhận thấy GroundingDINO có hành vi \textbf{không trung thực với text prompt} (prompt unfaithfulness): do cơ chế cross-modal fusion cho phép mô hình ``neo'' vùng ảnh vào token ngôn ngữ theo nghĩa ngữ cảnh rộng, GroundingDINO có thể trả về detection box ngay cả khi prompt không khớp với nội dung ảnh. Ví dụ minh họa tại Hình~\ref{fig:prompt_faithfulness}: khi prompt là \texttt{"lion"} trên ảnh chứa chim hồng hạc, GroundingDINO vẫn phát hiện và đếm được 14 đối tượng - hoàn toàn sai về mặt ngữ nghĩa. Ngược lại, YOLO-World sử dụng CLIP embedding để so sánh trực tiếp cosine similarity giữa visual patch và text class; khi prompt là \texttt{"lion"}, độ tương đồng với các patch chim quá thấp nên mô hình trả về \textit{không có đối tượng nào} (zero detections). Chỉ khi prompt đúng (\texttt{"bird"}), YOLO-World mới phát hiện và đếm chính xác. Tính chất \textit{faithful to prompt} này đặc biệt quan trọng trong ứng dụng thực tế, nơi người dùng cần hệ thống đếm \emph{đúng thứ họ yêu cầu}, không phải đếm bất kỳ vật thể nào có hình dạng tương tự.

\begin{figure}[H]
    \centering
    % Row 0: ảnh đầu vào
    \subcaptionbox{Ảnh đầu vào: chim hồng hạc\label{fig:pf_input}}{%
        \figureorplaceholder{figures/sample_bird.jpg}{0.55\textwidth}{Ảnh đầu vào gốc: chim hồng hạc.}%
    }
    \par\bigskip
    % Row 1: GroundingDINO + prompt "lion"
    \subcaptionbox{GroundingDINO: nhập prompt \texttt{"lion"}\label{fig:pf_gdino_input}}{%
        \figureorplaceholder{figures/sample_bird_GroundingDINO_prompt-lion.png}{0.48\textwidth}{GroundingDINO với prompt lion -- giao diện nhập.}%
    }\hfill
    \subcaptionbox{Kết quả: Count $= 14$ {\color{red}$\times$} (sai)\label{fig:pf_gdino_count}}{%
        \figureorplaceholder{figures/sample_bird_GroundingDINO_prompt-lion_count.png}{0.48\textwidth}{GroundingDINO với prompt lion -- kết quả đếm sai.}%
    }
    \par\bigskip
    % Row 2: YOLO-World + prompt "lion"
    \subcaptionbox{YOLO-World: nhập prompt \texttt{"lion"}\label{fig:pf_yolo_lion_input}}{%
        \figureorplaceholder{figures/sample_bird_YoloWorld_prompt-lion.png}{0.48\textwidth}{YOLO-World với prompt lion -- giao diện nhập.}%
    }\hfill
    \subcaptionbox{Kết quả: No objects detected {\color{green!60!black}$\checkmark$}\label{fig:pf_yolo_lion_nocount}}{%
        \figureorplaceholder{figures/sample_bird_YoloWorld_prompt-lion_no-count.png}{0.48\textwidth}{YOLO-World với prompt lion -- không phát hiện đối tượng.}%
    }
    \par\bigskip
    % Row 3: YOLO-World + prompt "bird"
    \subcaptionbox{YOLO-World: nhập prompt \texttt{"bird"}\label{fig:pf_yolo_bird_input}}{%
        \figureorplaceholder{figures/sample_bird_YoloWorld_prompt-bird.png}{0.48\textwidth}{YOLO-World với prompt bird -- giao diện nhập.}%
    }\hfill
    \subcaptionbox{Kết quả: Count $= 14$ {\color{green!60!black}$\checkmark$} (đúng)\label{fig:pf_yolo_bird_count}}{%
        \figureorplaceholder{figures/sample_bird_YoloWorld_prompt-bird_count.png}{0.48\textwidth}{YOLO-World với prompt bird -- đếm chính xác.}%
    }
    \caption{Minh họa tính chất \textit{prompt faithfulness} của YOLO-World so với GroundingDINO. Ảnh đầu vào chứa chim hồng hạc. GroundingDINO với prompt \texttt{"lion"} vẫn trả về Count $= 14$ (sai). YOLO-World với prompt \texttt{"lion"} trả về \textit{zero detections} nhờ CLIP cosine similarity thấp; chỉ với prompt đúng \texttt{"bird"} mới đếm chính xác Count $= 14$.}
    \label{fig:prompt_faithfulness}
\end{figure}
...
\section{Tổng hợp bốn cấu hình thực nghiệm}
\label{sec:config-summary}

Bảng~\ref{tab:config_summary} tóm tắt bốn cấu hình hệ thống được đánh giá trong đồ án, giúp người đọc nắm nhanh sự khác nhau giữa các cấu hình trước khi xem kết quả chi tiết tại Chương~\ref{ch:experiments}.
...
\hline
\textbf{Cấu hình} & \textbf{Detector} & \textbf{Rich Prompt} & \textbf{Re-rank (CLIP)} & \textbf{top-k} & \textbf{Ưu điểm chính} \\
\hline
Baseline & GDino & \texttimes & \texttimes & 3 & Tái hiện trung thực VA-Count gốc; điểm tham chiếu. \\
\hline
+Rich Prompt & GDino & \checkmark & \checkmark & 3 & Exemplar chất lượng cao hơn nhờ mô tả Gemini + CLIP re-rank. \\
\hline
+YOLO-World & YOLO & \texttimes & \texttimes & 3 & Tốc độ nhanh hơn $\sim$20$\times$; prompt faithful nhờ CLIP cosine. \\
\hline
+YOLO+RP & YOLO & \checkmark & \checkmark & 3 & Kết hợp tốc độ YOLO-World và ngữ nghĩa phong phú từ Rich Prompt. \\
\hline
\end{tabular}
\caption{Tổng hợp bốn cấu hình thực nghiệm: Detector (GDino = GroundingDINO, YOLO = YOLO-World), Rich Prompt (bật/tắt Gemini expansion + CLIP re-rank), top-k (số exemplar dương). Kết quả định lượng xem Bảng~\ref{tab:main_results}.}
\label{tab:config_summary}
\end{table}

\bigskip
\noindent\textit{\textbf{Kết chương~2.} Chương này đã trình bày toàn bộ kiến trúc hệ thống -- từ VA-Count baseline (EEM + NSM) đến hai hướng mở rộng (Rich Prompts, YOLO-World) -- và tóm gọn bốn cấu hình sẽ được đánh giá. Chương~3 tiếp theo sẽ trình bày kết quả định lượng và định tính của các cấu hình này trên bộ dữ liệu FSC-147.}
...
\begin{itemize}
    ...
    \item \textbf{YOLO-World đơn lẻ có MAE cao hơn baseline} (19,03 so với 17,99). Nguyên nhân chính là mô hình nhẹ này, dù nhanh hơn đáng kể, có năng lực nhận dạng ngữ nghĩa yếu hơn GroundingDINO, dẫn đến nhiễu phát hiện (detection noise) cao hơn ở giai đoạn trích xuất mẫu. Cụ thể, YOLO-World sử dụng chiến lược Prompt-then-Detect (mã hóa ngoại tuyến text prompt), nên không có cơ chế tương tác thị giác-ngôn ngữ (vision-language interaction) tại thời điểm suy luận, dẫn đến khả năng phân biệt đối tượng mục tiêu và nhiễu nền kém hơn so với GroundingDINO. Tuy nhiên, cần lưu ý rằng MAE cao hơn của YOLO-World \emph{không xuất phát từ false positive} (phát hiện nhầm lớp không đúng): trái lại, tính chất \textit{faithful-to-prompt} khiến YOLO-World có ít false positive hơn GroundingDINO --- như minh họa tại Hình~\ref{fig:prompt_faithfulness}, YOLO-World từ chối phát hiện hoàn toàn khi prompt sai. Nguyên nhân thực sự của MAE cao hơn là YOLO-World \textbf{bỏ sót} (false negative) một số đối tượng đúng mà GroundingDINO nhặt được nhờ soft-matching ngữ nghĩa rộng. Trên FSC147, vì text prompt luôn là tên lớp chính xác, tính chất faithful-to-prompt không đem lại lợi thế về số liệu MAE; nhưng trong triển khai thực tế với prompt không chính xác, YOLO-World sẽ cho kết quả đáng tin cậy và có nghĩa hơn đáng kể.
...
\bigskip
\noindent\textit{\textbf{Kết chương~3.} Chương này đã trình bày toàn bộ kết quả định lượng và định tính trên FSC-147, phân tích ảnh hưởng của từng thành phần và phân loại các trường hợp lỗi sai. Chương~4 tiếp theo sẽ giới thiệu ứng dụng demo Streamlit và đánh giá định tính trên các ảnh thực tế ngoài tập dữ liệu.}
...
\bigskip
\noindent\textit{\textbf{Kết chương~4.} Chương này đã trình bày ứng dụng demo Streamlit cho phép kiểm thử hệ thống trực quan với mọi ảnh đầu vào tùy ý. Chương~5 tiếp theo sẽ tổng kết toàn bộ kết quả, phân tích giả thuyết bão hòa ngữ nghĩa và đề xuất hướng phát triển tiếp theo.}
...
\bigskip
\noindent\textit{\textbf{Kết chương~5.} Chương này đã tổng kết các kết quả đạt được, làm rõ giả thuyết bão hòa ngữ nghĩa và vạch ra các hướng phát triển tương lai. Danh sách tài liệu tham khảo đầy đủ được trình bày ở phần tiếp theo.}
